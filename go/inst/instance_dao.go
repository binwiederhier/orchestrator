...
func ReadTopologyInstanceBufferable(instanceKey *InstanceKey, bufferWrites bool, latency *stopwatch.NamedStopwatch) (inst *Instance, err error) {
	...
	err = sqlutils.QueryRowsMap(db, "show slave status", func(m sqlutils.RowMap) error {
		...
		instance.ReplicationIOThreadRunning = instance.ReplicationIOThreadState.IsRunning()
		if isMaxScale110 {
			// Covering buggy MaxScale 1.1.0
			instance.ReplicationIOThreadRunning = instance.ReplicationIOThreadRunning && (m.GetString("Slave_IO_State") == "Binlog Dump")
		}
		instance.ReplicationSQLThreadRunning = instance.ReplicationSQLThreadState.IsRunning()
...
func readInstanceRow(m sqlutils.RowMap) *Instance {
	...
	instance.IsDetachedMaster = instance.MasterKey.IsDetached()
	instance.ReplicationSQLThreadRunning = m.GetBool("slave_sql_running")
	instance.ReplicationIOThreadRunning = m.GetBool("slave_io_running")
...
func mkInsertOdkuForInstances(instances []*Instance, instanceWasActuallyFound bool, updateLastSeen bool) (string, []interface{}, error) {
	...
	for _, instance := range instances {
		...
		args = append(args, instance.ReplicationSQLThreadRunning)
		args = append(args, instance.ReplicationIOThreadRunning)
